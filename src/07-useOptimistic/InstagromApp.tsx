...
export const InstagromApp = () => {
    //!useTransition: Hace actualizaciones que no bloqueen el ui
    //? en este caso el startTransition es conocido como una accion, cuando se termine esa accion el isPending regresa a ser falso, 
    //? solo es verdadero mientras se hace esa accion
    ...
    //!useOptimistic: hace actualizaciones exitosas antes de tiempo, pero si ya la respuesta falla se puede hacer un rollback
    const [optimisticComments, addOptimisticComment] = useOptimistic( comments, (currentCommments, newCommentText:string)=>{
        lastId++;
        return [...currentCommments,{
            id: lastId,
            text: newCommentText,
            optimistic: true
        }]
    ...
    const handleAddComment = async (formData:FormData) => {
        ...
        //No bloquea el ui mientras se ejecutan
        startTransition(async()=>{
            // Simular la peticion http al servidor
            await new Promise((resolve) => setTimeout(resolve, 3000));     

            //En el caso de que funcione  
            // setComments(prev => [
            //     ...prev,
            //     {
            //         id: new Date().getTime(),
            ...
            //     }
            // ]);
            //Codigo para simular el fallo
            //? reversa el commentario que se estaba intentando enviar
